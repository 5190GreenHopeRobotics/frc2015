...
public class AHRS extends IMU {
	...
	/**
	 * Constructs the AHRS class, overriding the default update rate with a
	 * custom rate which may be from 4 to 60, representing the number of updates
	 * per second sent by the navX MXP.
	 * 
	 * Note that increasing the update rate may increase the CPU utilization.
	 * 
	 * @param serial_port
	 *            SerialPort object to use
	...
	public AHRS(SerialPort serial_port, byte update_rate_hz) {
		...
		is_moving = is_rotating = altitude_valid = is_magnetometer_calibrated = magnetic_disturbance = false;
		resetDisplacement();
	}
	...
	protected int decodePacketHandler(byte[] received_data, int offset,
		...
		int packet_length = AHRSProtocol.decodeAHRSUpdate(received_data,
				offset, bytes_remaining, ahrs_update_data);
		if (packet_length > 0) {
	...
	/**
	 * Indicates if the navX MXP is currently detecting motion, based upon the x
	...
	/**
	 * Indicates if the navX MXP is currently detecting yaw rotation, based upon
	 * whether the change in yaw over the last second exceeds the
	 * "Rotation Threshold."
	 * 
	 * Yaw Rotation can occur either when the navX MXP is rotating, or when the
	 * navX MXP is not rotating, but the current gyro calibration is
	 ...
	 * 
	 * NOTE: This value is only valid for a navX MXP Aero. To determine whether
	...
	/**
	 * Returns the current altitude, based upon calibrated readings from the
	 * onboard pressure sensor, and the currently-configured sea-level
	 * barometric pressure. This value is in units of meters.
	 * 
	 * NOTE: This value is only valid for a navX MXP Aero. To determine whether
	 * this value is valid, see isAltitudeValid().
	 * 
	 ...
	 * compass heading, and magnetic disturbance detection. Note that the navX
	 * MXP magnetometer calibration procedure is required in order to achieve
	 * valid 9-axis headings.
	 * 
	 * The 9-axis Heading represents the navX MXP's best estimate of current
	 ...
	 * Angle is read and the navX MXP has recently rotated less than the Compass
	...
	private void setAHRSData(AHRSProtocol.AHRSUpdate ahrs_update) {
		synchronized (this) { // synchronized block
			...
			// this.is_moving = (((ahrs_update.sensor_status &
			// AHRSProtocol.NAVX_SENSOR_STATUS_MOVING) != 0) ? true
			// : false);
			// this.is_rotating = (((ahrs_update.sensor_status &
			// AHRSProtocol.NAVX_SENSOR_STATUS_YAW_STABLE) != 0) ? true
			// : false);
			// this.altitude_valid = (((ahrs_update.sensor_status &
			// AHRSProtocol.NAVX_SENSOR_STATUS_ALTITUDE_VALID) != 0) ? true
			// : false);
			// this.is_magnetometer_calibrated = (((ahrs_update.cal_status &
			// AHRSProtocol.NAVX_CAL_STATUS_MAG_CAL_COMPLETE) != 0) ? true
			// : false);
			// this.magnetic_disturbance = (((ahrs_update.sensor_status &
			// AHRSProtocol.NAVX_SENSOR_STATUS_MAG_DISTURBANCE) != 0) ? true
			// : false);

			updateDisplacement(this.world_linear_accel_x,
					this.world_linear_accel_y, update_rate_hz, this.is_moving);
	...
	private void updateDisplacement(float accel_x_g, float accel_y_g,
		...
		if (is_moving) {
			...
			for (int i = 0; i < 2; i++) {
				...
				curr_velocity_m_s[i] = last_velocity[i]
						+ (accel_m_s2[i] * sample_time);
				displacement[i] += last_velocity[i]
						+ (0.5f * accel_m_s2[i] * sample_time * sample_time);
