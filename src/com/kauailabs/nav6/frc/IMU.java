...
public class IMU extends SensorBase implements PIDSource, LiveWindowSendable,
	...
	public void run() {
		...
		try {
			...
			serial_port.flush();
			port_reset_count++;
			// SmartDashboard.putNumber("nav6_PortResets",
		...
		while (!stop) {
			try {
				...
				if (bytes_read > 0) {
					...
					while (i < bytes_read) {
						...
						if (received_data[i] != IMUProtocol.PACKET_START_CHAR) {
							/*
							 * Skip over received bytes until a packet start is
							 * detected.
							 */
							i++;
							discarded_bytes_count++;
							// SmartDashboard.putNumber("nav6 Discarded Bytes",
							...
							if ((bytes_remaining > 2)
								...
								/*
								 * Binary packet received; next byte is packet
								 * length-2
								 */
								...
								while (bytes_remaining < total_expected_binary_data_bytes) {
									...
									/*
									 * end-of-line character. Continue to
									 * receive
									 */
									/*
									 * more data until entire packet is
									 * received.
									 */
									byte[] additional_received_data = serial_port
											.read(256);
									byte_count += additional_received_data.length;
									bytes_remaining += additional_received_data.length;

									/*
									 * Resize array to hold existing and new
									 * data
									 */
									byte[] c = new byte[received_data.length
											+ additional_received_data.length];
									if (c.length > 0) {
										System.arraycopy(received_data, 0, c,
												0, received_data.length);
										System.arraycopy(
												additional_received_data, 0, c,
												received_data.length,
												additional_received_data.length);
										received_data = c;
									} else {
										/*
										 * Timeout waiting for remainder of
										 * binary packet
										 */
										i++;
										bytes_remaining--;
						...
						if (packet_length > 0) {
							...
							if ((last_valid_packet_time - last_second_start_time) > 1.0) {
								// SmartDashboard.putNumber("nav6 UpdatesPerSec",
					...
					if ((packets_received == 0) && (bytes_read == 256)) {
						// Workaround for issue found in SerialPort
						// implementation:
					...
					/*
					 * If receiving data, but no valid packets have been
					 * received in the last second
					 */
					/*
					 * the navX MXP may have been reset, but no exception has
					 * been detected.
					 */
					/*
					 * In this case , trigger transmission of a new
					 * stream_command, to ensure the
					 */
				...
				stream_response_received = false;
				timeout_count++;
				// SmartDashboard.putNumber("nav6 Serial Port Timeouts",
				// (double)timeout_count);
				// SmartDashboard.putString("LastNavExceptionBacktrace",ex.getStackTrace().toString());
				// SmartDashboard.putString("LastNavException", ex.getMessage()
				// + "; " + ex.toString());
			}
		}
	}
